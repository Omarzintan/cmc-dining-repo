...
[
  collins-patches
  food-truck-patches
  new-food-patches ;;represents added food truck or eating area
  test-patches
  campus-patches
  max-people-collins
  ...
  top-side-of-new-food-patches
  bottom-side-of-new-food-patches
  right-side-of-new-food-patches
  collins-attendance
  food-truck-attendance
  new-food-attendance
  collins-crowd-threshold
  food-trucks-crowd-threshold
  new-food-crowd-threshold
  collins-crowdedness-duration
  food-truck-crowdedness-duration
  new-food-crowdedness-duration
]
turtles-own
[
...
to setup
  ...
  set campus-patches patches with [(pxcor < 40 and pxcor > -70) or (pxcor >= 40 and (pycor < 70 and pycor > -60)) or ( pxcor <= -70 and (pycor > 20 or pycor < -20))]
  ask campus-patches [set pcolor white]
  ;; set patches for new food area
  set new-food-patches patches with [pxcor < -70 and (pycor < 20 and pycor > -20)]
  ask new-food-patches [set pcolor 117]
  ...
  create-category-of-students black new-food-patches
  create-category-of-students blue campus-patches ;;campus patches is just a placeholder,
                                                    ;;the location for blue students is chosen
                                                    ;;based on the nearest eating location to them.
  ...
  set new-food-crowd-threshold 62
...
to go
  ...
  if count turtles-on new-food-patches > max-people-new-food
  [set max-people-new-food count turtles-on new-food-patches]
...
to create-category-of-students [category-color location]
  ...
  create-turtles 325 [
    ...
    [
      let closest-patch min-one-of (patches with [pcolor = 18 or pcolor = 68 or pcolor = white])[distance myself]
      if [pcolor] of closest-patch = 18
      [
        set location-preference collins-patches
      ]

      if [pcolor] of closest-patch = 68
      [
        set location-preference food-truck-patches
      ]

      if [pcolor] of closest-patch = white
      [
        set location-preference new-food-patches
      ]
    ]
...
to search-for-food [destination-patches]
  ...
  [
     ...
     if destination-patches = new-food-patches
     [
       if new-food-crowdedness >= new-food-crowd-threshold
       [
         set location-preference collins-patches
       ]
     ]
...
to calculate-crowdedness
  ...
  set food-truck-attendance count turtles-on food-truck-patches
   ;; get how many turtles are at the new food truck
  set new-food-attendance count turtles-on new-food-patches
  ...
  if new-food-attendance >= new-food-crowd-threshold
  [ set new-food-crowdedness-duration (new-food-crowdedness-duration + 1)]
  ...
  ask-concurrent right-side-of-new-food-patches [ set new-food-crowdedness round (new-food-attendance * (1 - 0.005) ^ distance-from-location) ]
  ask-concurrent top-side-of-new-food-patches [ set new-food-crowdedness round (new-food-attendance * (1 - 0.005) ^ distance-from-location) ]
  ask-concurrent bottom-side-of-new-food-patches [ set new-food-crowdedness round (new-food-attendance * (1 - 0.005) ^ distance-from-location) ]
end
...
to create-crowdedness-range
  ...
  set top-side-of-food-truck-patches patches with [(pxcor >= 40 and pycor <= -30 and pycor > -60)]
   ...
   ;; Create patches around the new food from which agents can receive info on how crowded the new food is.
  set right-side-of-new-food-patches patches with [(pxcor > -70 and pxcor <= -40 and pycor <= 50 and pycor >= -50)]
  set top-side-of-new-food-patches patches with [(pxcor <= -70 and pycor <= 50 and pycor > 20)]
  set bottom-side-of-new-food-patches patches with [(pxcor <= -70 and pycor >= -50 and pycor < -20)]
  ...
  ask right-side-of-new-food-patches [set pcolor grey]
  ask top-side-of-food-truck-patches [set pcolor grey]
  ask top-side-of-new-food-patches [set pcolor grey]
  ask bottom-side-of-new-food-patches [set pcolor grey]
  ...
  ask-concurrent right-side-of-new-food-patches [ set distance-from-location max(list 0 ((round distance patch -85 0) - 13))]
  ask-concurrent top-side-of-new-food-patches [ set distance-from-location max(list 0 ((round distance patch -85 0) - 13))]
  ask-concurrent bottom-side-of-new-food-patches [ set distance-from-location max(list 0 ((round distance patch -85 0) - 13))]
end
