% Measuring When It's Crowded-Control Measures' Influence (DE)
%import data
Collins_output = readmatrix('eating-duration-controlmeasure Experiments Collins-spreadsheet.csv');
...
%Food Trucks
%extract data:
Food_Trucks_output = readmatrix('eating-duration-controlmeasure Experiments Food Truck-spreadsheet');
...
title("Number of Students at Collins Every Five Ticks (Eating Duration)")
...
hold on
y2=yline(124,'--','Collins Threshold', LineWidth=1);
y2.LabelHorizontalAlignment = 'left';
%line(Final_output_ticks_v, FT_threshold*ones(size(Final_output_ticks_v)),'Color','g')
xlabel('Ticks')
ylabel('Students at Food Trucks')
title("Number of Students at Food Trucks Every Five Ticks (Eating Duration)")
...
title("Average Number of Students at Each Food Source Every Five Ticks (Eating Duration)")
