% Measuring When It's Crowded-Control Measures' Influence (New Food-NF)
%import data
Collins_output = readmatrix('foodtrucks-controlmeasure Experiment Collins-spreadsheet');
...
%Food Trucks
%extract data:
Food_Trucks_output = readmatrix('foodtrucks-controlmeasure Experiment Food Truck-spreadsheet');
...
%New Foods
%extract data
New_Food_output = readmatrix('foodtrucks-controlmeasure Experiment New Food Source-spreadsheet');
New_Food_students_base=New_Food_output(9:5:end,2:3:31);
%transpose the matrix
transposed_New_Food_students_base=(New_Food_students_base)'
%create a vector with each trial as a vector value
New_Food_students_base_v=reshape(transposed_New_Food_students_base, [1,410]);
%transpose the vector
Final_New_Food_student_base_v=(New_Food_students_base_v)'
...
title("Number of Students at Collins Every Five Ticks (New Food)")
...
y4=yline(124,'--','Food Truck Threshold', LineWidth=1);
y4.LabelHorizontalAlignment = 'left';
xlabel('Ticks')
ylabel('Students at Food Trucks')
title("Number of Students at Food Trucks Every Five Ticks (New Food)")

%boxplot for New Food number of students 
figure
boxplot(Final_New_Food_student_base_v,Final_output_ticks_v)
hold on
%line(Final_output_ticks_v, NF_threshold*ones(size(Final_output_ticks_v)),'Color','r')
y3=yline(62,'--','New Food Threshold',LineWidth=1)
y3.LabelHorizontalAlignment = 'left';
xlabel('Ticks')
ylabel('Students at New Food')
title("Number of Students at New Food Every Five Ticks (New Food)")
...
%New Food Graphs:
mean_new_food_students_base=mean(transposed_New_Food_students_base)
...
h3 = plot(Ticks_number,mean_new_food_students_base, 'color', '#7E2F8E','LineWidth',1)
...
title("Average Number of Students at Each Food Source Every Five Ticks (New Food)")
legend([h1(1), h2(1), h3(1)], 'Collins', ' Food Trucks', 'New Food')