% Measuring When It's Crowded-Behaviors' Influence (Base Model)
%import data
Collins_output = readmatrix('base-model Experiment - Collins-spreadsheet.csv');
...
%Food Trucks
%extract data:
Food_Trucks_output = readmatrix('base-model Experiment - Food Truck-spreadsheet');
...
hold on
y1=yline(210,'--','Collins Threshold',LineWidth=1);
...
title("Number of Students at Collins Every Five Ticks (Base Model)") 
...
title("Number of Students at Food Trucks Every Five Ticks (Base Model)")
...
title("Average Number of Students at Each Food Source Every Five Ticks (Base Model)")
